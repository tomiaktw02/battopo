// =========================================================
// Battopo - ?›æ“¬å¯µç‰©?Šæˆ²ï¼ˆå??¯é€²å??ˆï?
// =========================================================

(() => {
    'use strict';

    const IS_DEBUG = false; // [VERSION_FLAG] ?§åˆ¶?µéŒ¯?‡ä»¤?¯å¦?¯ç”¨
    ...
    function registerToDex(formId) {
        ...
        saveDex(dex);
        addMsg(t('msg_dex_new') + `${formInfo.emoji} ${formInfo.name}ï¼`, 'success');
    }
    ...
    function exportSaveFile() {
        ...
        addMsg(`?’¾ å­˜æ?å·²ä?è¼? battopo_${formName}_${dateStr}.json`, 'success');
    }
    ...
    function renderActions() {
        ...
        const actions = state.stage === STAGE_EGG
            ...
            : [
                ...
                { id: 'clean',  emoji: '?š¿', label: 'CLEAN',  desc: t('ui_clean_desc') },
                { id: 'play',   emoji: '?Œï?', label: 'PLAY',   desc: t('ui_play_desc') },
                { id: 'battle', emoji: '?”ï?', label: 'BATTLE', desc: t('ui_battle_desc') },
                { id: 'rename', emoji: '?ï?', label: 'RENAME', desc: t('ui_rename_desc') },
            ];
    ...
    function openClean() {
        ...
        addMsg(t('msg_clean_start') || '?§¹ ?¼ç¾?²èŸ²ï¼è?è¼¸å…¥? ç??±æ??ç¨±??, 'info');
        renderPoops();
    }
    ...
    function handleCleaningInput(cmd) {
        ...
        if (cmd === state.currentPest.id.toLowerCase()) {
            ...
            // Wrong name
            addMsg(t('msg_clean_wrong') || '???ç¨±?¯èª¤ï¼è??è©¦ä¸€æ¬¡ã€?, 'error');
    ...
    async function startBattle(forcedOpponent = null) {
        ...
        // ?•ç??°é¬¥?æ???        if (log.preBattle && log.preBattle.length > 0) {
    ...
    function setBattleMsg(txt) {
        if (!state.battleDebug) {
            ...
            const replacements = [
                { reg: /?—åˆ°\s*\d+\s*é»žå‚·å®?g, val: t('msg_battle_dmg') },
                { reg: /HP\s*-\d+/gi, val: t('msg_battle_dmg') },
                { reg: /HP\s*\+\d+/gi, val: t('msg_battle_heal') },
                { reg: /?¢å¾©äº†\s*\d+\s*é»ž\s*HP/g, val: t('msg_battle_heal') },
                { reg: /(?»æ??›|atk).*?\+\d+/gi, val: t('msg_battle_atk_up') },
                { reg: /(?»æ??›|atk).*?-\d+/gi, val: t('msg_battle_atk_down') },
                { reg: /(?²ç¦¦?›|def).*?\+\d+/gi, val: t('msg_battle_def_up') },
                { reg: /(?²ç¦¦?›|def).*?-\d+/gi, val: t('msg_battle_def_down') },
                { reg: /(?Ÿåº¦|spd).*?\+\d+/gi, val: t('msg_battle_spd_up') },
                { reg: /(?Ÿåº¦|spd).*?-\d+/gi, val: t('msg_battle_spd_down') },
                { reg: /?·å®³\s*\+\s*\d+/g, val: t('msg_battle_dmg_up') }
            ];
    ...
    async function playAction(action) {
        ...
        // ?•ç??€?½è§¸?¼æ—¥èª?        if (action.triggers && action.triggers.length > 0) {
            for (const trigger of action.triggers) {
                if (state.battleDebug) {
    ...
    function endBattle(playerWon) {
        ...
        // å¦‚æ??‹å?äº†è©³ç´?logï¼Œå?å°‡æˆ°é¬¥è??¯å???txt
    ...
    function restart() {
        ...
        state = defaultState();
        leaveOverlay.classList.add('hidden');
        msgLog.innerHTML = ''; // æ¸…ç©ºè¨Šæ¯ç´€??        addMsg(t('msg_egg_intro'), 'success');
        save();
        renderAll();
    }
