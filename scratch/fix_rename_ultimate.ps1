...
foreach ($file in $files) {
    ...
    if ($sizeToId.ContainsKey($size)) {
        ...
        $targetPath = Join-Path $imagesDir $newName
        
        # We don't care if the target is already there, we will overwrite or temp it.
        # But to be clean, let's just use a two-pass approach.
    }
}

# Pass 1: Rename everything to a unique UUID-like or just very long temp prefix
$i = 0
foreach ($file in $files) {
    $tempName = "ultimate_temp_$i.png"
    Rename-Item -Path $file.FullName -NewName $tempName
    $i++
}

# Pass 2: Rename back from temp to official
$tempFiles = Get-ChildItem -Path $imagesDir -Filter ultimate_temp_*.png
foreach ($file in $tempFiles) {
    $size = $file.Length.ToString()
    if ($sizeToId.ContainsKey($size)) {
        $newName = $sizeToId[$size] + ".png"
        Write-Host "Restoring size $size to $newName"
        Rename-Item -Path $file.FullName -NewName $newName
    } else {
        Write-Host "CRITICAL: Could not find mapping for size $size ($($file.Name))"
    }
}
